# ノード構築
各ノードにてOSインストール後に以下のセットアップ作業を行います。
...
キーペア作成後、作業用PCにて各ノードに設定する公開鍵の中身を確認しておきます。
```
cat ~/.ssh/id_rsa.pub
ssh-rsa ~~~ user@local-pc  ## この出力結果を各ノードに設定する。
```

続いて、セットアップ対象のノードにログインして、先ほど作成したSSH公開鍵を設定します。
```
ssh ubuntu@(ノードのIPアドレス)
...
## sudoのパスワード無効化
ノード内で下記コマンドを実行して、sudo権限を付与したコマンドをパスワード入力なしで実行できるようにします。
```
echo "ubuntu ALL=(ALL) NOPASSWD:ALL" | sudo tee /etc/sudoers.d/99-config
```

## netplan編集
ノード内で下記コマンドをしてネットワークの設定を行います。
